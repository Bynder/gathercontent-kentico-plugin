...
using GatherContentConnector.CMSModules.GatherContentConnector.Import;

[assembly: RegisterCustomClass("ImportGridExtender", typeof(ImportGridExtender))]
    ...
    public class ImportGridExtender : ControlExtender<UniGrid>
    {
        ...
        private void BindFilters(string key, FiltersModel filters)
        {
            ...
            if (key == "projectId")
            {
                ...
                if (filters != null && filters.Projects != null && filters.Projects.Count > 0)
                {
                    field.SetListItems(filters.Projects.Select(x => new ListItem(x.Name, x.Id)).ToArray());
                    if (filters.Projects.Count == 1)
                    {
                        this.projectId = filters.Projects.First().Id;
                        field.Value = this.projectId;
                        field.IsChange = true;
                    }
                }
            }
            ...
            ref int totalRecords)
        {
            try
            {
                ...
                var filteredEntries = this.GetFilteredEntries(this.Control.FilterForm);

                var table = new DataTable("MayThisTableHaveIndex0");
                table.Columns.AddRange(
                    new[]
                        {
                            new DataColumn("Id"), new DataColumn("projectId"), new DataColumn("status"), new DataColumn("name"), new DataColumn("temaplateName"),
                            new DataColumn("lastModified"), new DataColumn("path")
                        });
                ...
                if (filteredEntries != null)
                {
                    foreach (var item in filteredEntries)
                    {
                        table.Rows.Add(
                            item.GcItem.Id,
                            this.projectId,
                            string.Format(string.IsNullOrWhiteSpace(item.Status.Color) ? item.Status.Name : item.Status.Color + ":" + item.Status.Name),
                            item.GcItem.Title,
                            item.GcTemplate.Name,
                            item.GcItem.LastUpdatedInGc,
                            item.Breadcrumb);
                    }
                }
        ...
        private List<ItemModel> GetFilteredEntries(FilterForm filterForm)
        {
            this._importManager = GCServiceLocator.Current.GetInstance<IImportManager>();

            if (!string.IsNullOrEmpty(filterForm.GetFieldValue("projectId") as string))
            {
                this.projectId = filterForm.GetFieldValue("projectId") as string;
            }
            ...
            var result = this._importManager.GetImportDialogModel(this.projectId);
        ...
        private void ImportActionHandler(object obj, EventArgs e)
        {
            ...
            {
                this.projectId = this.Control.FilterForm.GetFieldValue("projectId") as string;
                ...
                else
                {
                    ...
                    URLHelper.Redirect(URLHelper.AddParameterToUrl(URLHelper.AddParameterToUrl(elementUrl, parameterName, parameterValue), "projectId", this.projectId));
                }
            }
        }
        ...
        private void PageOnPreRender(object sender, EventArgs eventArgs)
        {
            ...
            ModuleCssRegistration.RegisterModuleStylesheet(this.Page, "GatherContentConnector", "Main.css");
        }
    }
}