...
namespace GatherContent.Connector.Managers.Managers
{
    ...
    public class UpdateManager : BaseManager, IUpdateManager
    {
        ...
        public List<ItemResultModel> UpdateItems(string itemId, List<UpdateListIds> models, string language)
        {
            ...
            {
                var gcItem = ItemsService.GetSingleItem(item.GCId).Data;
            ...
            foreach (var item in gcItems)
            {
                ...
                try
                {
                    ...
                    var cmsLink = ItemsRepository.GetCmsItemLink(language, cmsId);
                    ...
                    if (templateMapping != null)
                    {
                        ...
                        var fieldError = CheckFieldError(templateMapping, gcFields, files, itemResponseModel);
                ...
                catch (Exception ex)
                {
                    ...
                    itemResponseModel.IsImportSuccessful = false;
                    LogRepository.Log("Gather Content Connector", "UpdateManager", ex);
                }
                    ...
                    if (entity != null)
                    {
                        ...
                        if (gcItem.TemplateId.HasValue)
                        {
                            ...
                            var listItem = new UpdateItemModel
                            {
                                CmsId = cmsItem.Id,
                                Title = cmsItem.Title,
                                CmsLink = ItemsRepository.GetCmsItemLink(cmsItem.Language, cmsItem.Id),
                                GcLink = gcLink,
                                LastUpdatedInCms = lastUpdate.ToString(dateFormat, CultureInfo.CurrentUICulture),
                                ...
                                GcItem =
                                    new GcItemModel
                                    {
                                        ...
                                        Title = gcItem.Name,
                                        LastUpdatedInGc = TimeZoneInfo.ConvertTimeFromUtc(
                                                gcItem.Updated.Date,
                                                TimeZoneInfo.Local)
                                            .ToString(dateFormat),
                                    }
